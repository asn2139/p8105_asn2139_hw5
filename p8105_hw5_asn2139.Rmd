...
Function
```{r}
simul_1 = function(mu, samp_size=30, sigma=5) {
  
  sim_data = tibble(
    x = rnorm(mu,samp_size,sigma)
  )
  
  sim_data %>% 
    summarize(mu_hat = mean(x))%>%
mutate(
t_test = map(x, ~t.test(x, alternative = 'two.sided', paired = FALSE, conf.level = 0.95))
)
}
...
for (i in 1:6) {
  output[[i]] = rerun(5000, simul_1(mu_list[[i]])) %>% 
    bind_rows
}
```
